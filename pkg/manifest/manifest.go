...
func (mc *ManifestController) CreateManifest(namespace, url string) error {

	var client http.Client
	mc.logger.Info("Sakshi::url received", "URL", url)
	mc.logger.Info("Sakshi::namespace received", "Namespace", namespace)
	...
	return mc.createOrUpdateManifest(namespace, url, bodyBytes)
}

func (mc *ManifestController) createOrUpdateManifest(namespace, url string, bodyBytes []byte) error {
	...
	if existing != nil {
		...
		mc.logger.Info("Sakshi:: manifest crd already exists", "Manifest", existing)
		return nil

	} else {
		mc.logger.Info("manifest does not exist, creating", "ManifestName", "ss-ns", "Namespace", namespace)

		// Deserialize the manifest contents and fetch all the objects
		_, err := mc.CreateManifestCRD(namespace, url, bodyBytes)
...
func (mc *ManifestController) CreateManifestCRD(namespace, url string, data []byte) ([]*runtime.Object, error) {
	...
	var namespaceObj *core_v1.Namespace
	...
	mc.logger.Info("Sakshi:Manifest crd to be created", "Manifest", manifest)
	...
	mc.logger.Info("manifest created successfully", "ManifestName", "ss-ns")

	return nil, nil
}

func (mc *ManifestController) GetObjectsFromManifest(data []byte) ([]runtime.Object, error) {
	apiextensionsv1.AddToScheme(scheme.Scheme)
	apiextensionsv1beta1.AddToScheme(scheme.Scheme)
	adm_v1.AddToScheme(scheme.Scheme)
	apps_v1.AddToScheme(scheme.Scheme)
