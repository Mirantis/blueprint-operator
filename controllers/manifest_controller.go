...
func (r *ManifestReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	...
	instance := &blueprintv1alpha1.Manifest{}
	...
	if instance.ObjectMeta.DeletionTimestamp.IsZero() {
		...
		if !controllerutil.ContainsFinalizer(instance, finalizerName) {
			...
			if err := r.Update(ctx, instance); err != nil {
				...
				r.updateStatus(ctx, logger, key, blueprintv1alpha1.TypeComponentUnhealthy, "failed to update manifest object with finalizer", fmt.Sprintf("failed to update manifest object with finalizer : %s", err))
				return ctrl.Result{}, err
			}
		...
		if controllerutil.ContainsFinalizer(instance, finalizerName) {
			...
			if err = r.DeleteManifestObjects(ctx, instance.Spec.Objects); err != nil {
				...
				r.updateStatus(ctx, logger, key, blueprintv1alpha1.TypeComponentUnhealthy, "failed to delete manifest objects", fmt.Sprintf("failed to delete manifest objects : %s", err))
				return ctrl.Result{}, err
			}
			...
			if err = r.Update(ctx, instance); err != nil {
				...
				r.updateStatus(ctx, logger, key, blueprintv1alpha1.TypeComponentUnhealthy, "failed to remove finalizer", fmt.Sprintf("failed to remove finalizer : %s", err))
				return ctrl.Result{}, err
			}
		}
	...
	if instance.Spec.Checksum == instance.Spec.NewChecksum {
		...
		if err = r.updateManifestStatus(ctx, logger, req.NamespacedName, instance.Spec.Objects); err != nil {
			...
			r.updateStatus(ctx, logger, key, blueprintv1alpha1.TypeComponentUnhealthy, "failed to update manifest status", fmt.Sprintf("failed to update manifest status : %s", err))
			return ctrl.Result{}, err
		}
		return ctrl.Result{}, nil
	}
	...
	if (instance.Spec.Checksum != instance.Spec.NewChecksum) && (instance.Spec.NewChecksum != "") {
		...
		updatedCRD := blueprintv1alpha1.Manifest{
		...
		if err = r.Update(ctx, &updatedCRD); err != nil {
			...
			r.updateStatus(ctx, logger, key, blueprintv1alpha1.TypeComponentUnhealthy, "failed to update manifest crd while update operation ", fmt.Sprintf("failed to update manifest crd while update operation  : %s", err))
			return ctrl.Result{}, err
		}
		...
		if err = r.UpdateManifestObjects(req, ctx, instance); err != nil {
			...
			r.updateStatus(ctx, logger, key, blueprintv1alpha1.TypeComponentUnhealthy, "failed to update manifest ", fmt.Sprintf("failed to update manifest  : %s", err))
			return ctrl.Result{}, err
		}
...
func (r *ManifestReconciler) SetupWithManager(mgr ctrl.Manager) error {
	...
	if err := mgr.GetFieldIndexer().IndexField(context.Background(), &blueprintv1alpha1.Manifest{}, manifestUpdateIndex, func(rawObj client.Object) []string {
		manifest := rawObj.(*blueprintv1alpha1.Manifest)
	...
	return ctrl.NewControllerManagedBy(mgr).
		For(&blueprintv1alpha1.Manifest{}).
		Watches(
			&appsv1.DaemonSet{},
...
func (r *ManifestReconciler) findAssociatedManifest(ctx context.Context, obj client.Object) []reconcile.Request {
	attachedManifestList := &blueprintv1alpha1.ManifestList{}
...
func (r *ManifestReconciler) CreateManifestObjects(ctx context.Context, manifestNamespacedName types.NamespacedName, logger logr.Logger, data []byte) error {
	...
	var manifestObjs []blueprintv1alpha1.ManifestObject
	for _, o := range objs {
		manifestObjs = append(manifestObjs, blueprintv1alpha1.ManifestObject{
	...
	crd := &blueprintv1alpha1.Manifest{}
...
func (r *ManifestReconciler) DeleteManifestObjects(ctx context.Context, objectList []blueprintv1alpha1.ManifestObject) error {
	logger := log.FromContext(ctx)
...
func (r *ManifestReconciler) UpdateManifestObjects(req ctrl.Request, ctx context.Context, existing *blueprintv1alpha1.Manifest) error {
	logger := log.FromContext(ctx)
	...
	var newManifestObjs []blueprintv1alpha1.ManifestObject
	for _, o := range objs {
		newManifestObjs = append(newManifestObjs, blueprintv1alpha1.ManifestObject{
			Group:     o.GroupVersionKind().Group,
			Version:   o.GroupVersionKind().Version,
			Kind:      o.GetKind(),
			Name:      o.GetName(),
...
func (r *ManifestReconciler) findAndDeleteObsoleteObjects(req ctrl.Request, ctx context.Context, oldObjects []blueprintv1alpha1.ManifestObject, newObjects []blueprintv1alpha1.ManifestObject) {
	logger := log.FromContext(ctx)

	var obsolete []blueprintv1alpha1.ManifestObject
...
func (r *ManifestReconciler) updateManifestStatus(ctx context.Context, logger logr.Logger, namespacedName types.NamespacedName, objects []blueprintv1alpha1.ManifestObject) error {
	mc := pkgmanifest.NewManifestController(r.Client, logger)
...
func (r *ManifestReconciler) updateStatus(ctx context.Context, logger logr.Logger, namespacedName types.NamespacedName, typeToApply blueprintv1alpha1.StatusType, reasonToApply string, messageToApply ...string) error {
	logger.Info("Update status with type and reason", "TypeToApply", typeToApply, "ReasonToApply", reasonToApply)

	manifest := &blueprintv1alpha1.Manifest{}
	...
	nilStatus := blueprintv1alpha1.ManifestStatus{}
